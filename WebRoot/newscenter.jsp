...
<div class="zhong">  
  ...
  <div class="main">
    ...
    <div class="right">
      ...
      <div class="news">
        <div class="news_title">新闻列表</div>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <c:forEach items="${requestScope.mlist}" var="pl"> 
		 <tr>
            <td width="20%" height="18">${pl.title}</td>
            <td width="20%">${pl.author }</td>
            <td width="20%" align="center">${pl.ntime }</td>
            <td width="20%" align="center">${pl.source }</td>
            <td width="20%" align="center"><a href="getNewsByID.action?id=${pl.id }">详细</a></td>
          </tr>
         </c:forEach>        
       <tr>
            <td colspan="5" height="40"><s:if test="#request.pagetotal==0">没有记录！&nbsp;</s:if><s:else>&nbsp;当前第<s:property value="#request.pageNo"/>页
				<s:if test="#request.pageNo ==1"></s:if>
				<s:else><a href="getNewsList.action?pageNo=1&pagetotal=<s:property value="#request.pagetotal"/>">[首页]</a>&nbsp;
						<a href="getNewsList.action?pageNo=${pageNo-1}&pagetotal=${pagetotal}">[上一页]</a> 
				</s:else>
				<s:if test="#request.pageNo ==#request.pagetotal"></s:if>
				<s:else> <a href="getNewsList.action?pageNo=${pageNo+1}&pagetotal=${pagetotal}">[下一页]</a> 
				 		 <a href="getNewsList.action?pageNo=${pagetotal}&pagetotal=${pagetotal}">[尾页]</a>
