...
<div class="zhong">  
  ...
  <div class="main">
    ...
    <div class="right">
      ...
      <div class="news">
        <div class="news_title">法律法规列表</div>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <c:forEach items="${requestScope.mlist}" var="pl"> 
		 <tr>
            <td width="25%" height="18">${pl.title}</td>
            <td width="25%">${pl.organ }</td>
            <td width="25%" align="center">${pl.ldate }</td>
            <td width="25%" align="center"><a href="getLawByID.action?id=${pl.id }">详细</a></td>
          </tr>
         </c:forEach> 
<tr>
            <td colspan="4" height="40"><s:if test="#request.pagetotal==0">没有记录！&nbsp;</s:if><s:else>&nbsp;当前第<s:property value="#request.pageNo"/>页
				...
				<s:else><a href="getLawList.action?pageNo=1&pagetotal=<s:property value="#request.pagetotal"/>">[首页]</a>&nbsp;
						<a href="getLawList.action?pageNo=${pageNo-1}&pagetotal=${pagetotal}">[上一页]</a> 
				</s:else>
				<s:if test="#request.pageNo ==#request.pagetotal"></s:if>
				<s:else> <a href="getLawList.action?pageNo=${pageNo+1}&pagetotal=${pagetotal}">[下一页]</a> 
				 		 <a href="getLawList.action?pageNo=${pagetotal}&pagetotal=${pagetotal}">[尾页]</a>
